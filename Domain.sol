...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Eyciyo Name Service", "EyNS")
    {
        tld = _tld;
    ...
    function register(string calldata name) public payable {
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Eyciyo name service", "image": "data:image/svg+xml;base64,',
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 0.000000000005 * 10**17; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
        } else if (len == 4) {
            return 0.00000000003 * 10**17; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
            return 0.00000000001 * 10**17;
        }
    }
